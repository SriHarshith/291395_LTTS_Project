...
int addContact()
{
    ...
    {
        printf("\nContact Manager is out of storage\n");
        return 0;
    }
    else
    {
...
int displayAllContacts()
{
    if(presentSize == 0) 
    {
        printf("\nContact Manager is currently empty\n");
        return 0;
    }
    int k;
...
int searchContact(char *selection, search_t attribute)
{
    ...
    if(!flag) {
        printf("\nContact not found in the contact manager\n");
    }
...
int removeContact(char *phoneNumber)
{
    if(presentSize == 0) 
    {
        printf("\nContact Manager is empty, Cannot delete anything\n");
        return 0;
    }
